...
namespace StationpediaAscended.UI.StationPlanner
{
    ...
    public class StationPlannerWindow : MonoBehaviour
    {
        ...
        void OnDestroy()
        {
            ...
            // Clean up input state if we're destroyed while open
            KeyManager.RemoveInputState("StationNotepad");
        }
        
        void OnEnable()
        {
            // Subscribe to scene loading events for hotload detection
        ...
        /// <summary>
        /// Called when a scene is loaded (including F6 hotload).
        /// Re-selects the current file to refresh the display and prevent raw JSON from showing.
        /// </summary>
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            Debug.Log($"[Station Notepad] Scene loaded: {scene.name} (mode: {mode})");
            
            // Start a coroutine to check for raw JSON over several frames
            // The text might be set after this event fires
            StartCoroutine(CheckForRawJsonAfterLoad());
            
            // If the window was open, ensure input state is properly set
            if (IsOpen)
            {
        ...
        /// <summary>
        /// Coroutine that checks for raw JSON display over several frames after scene load.
        /// This handles cases where the text is set after the scene load event fires.
        /// </summary>
        private IEnumerator CheckForRawJsonAfterLoad()
        {
            // Check immediately and then over several frames
            for (int i = 0; i < 10; i++)
            {
                if (_blockEditor != null && _blockEditor.IsInitialized && _blockEditor.InputField != null)
                {
                    var currentText = _blockEditor.InputField.text;
                    if (LooksLikeSerializedDocument(currentText))
                    {
                        Debug.Log($"[Station Notepad] Detected raw JSON in editor (frame {i}) - re-deserializing");
                        _blockEditor.DeserializeDocument(currentText);
        ...
        /// <summary>
        /// Check if text looks like a serialized BlockEditorDocument JSON.
        /// Used to detect when raw JSON is being displayed instead of rendered content.
        /// </summary>
        private bool LooksLikeSerializedDocument(string text)
        {
            if (string.IsNullOrEmpty(text)) return false;
            
            // The serialized format has newlines/indentation, so we can't use simple StartsWith
            // Check for characteristic patterns of the BlockEditorDocument JSON format
        ...
        private void CreateToolbar(Transform parent)
        {
            ...
            _btnBullet.onClick.AddListener(() => ApplyLineStyle(LineStyle.Bullet));
            AddSimpleTooltip(_btnBullet.gameObject, "Bullet point - Toggle bullet style on current line");
        ...
        public void Show()
        {
            if (_windowCanvas != null)
            {
                ...
                // Check if the BlockEditor's text looks like serialized JSON (can happen after hotload)
                if (_blockEditor != null && _blockEditor.IsInitialized && _blockEditor.InputField != null)
                {
                    var currentText = _blockEditor.InputField.text;
                    if (LooksLikeSerializedDocument(currentText))
                    {
                        Debug.Log("[Station Notepad] Detected raw JSON in editor on Show - re-deserializing");
                        _blockEditor.DeserializeDocument(currentText);
                    }
        ...
        private void SelectFile(PlannerFile file)
        {
            ...
            if (_blockEditor != null && _blockEditor.IsInitialized && file != null)
            {
                ...
                // Use LooksLikeSerializedDocument for robust detection (handles newlines in JSON)
                if (LooksLikeSerializedDocument(file.Content))
                {
                    // Looks like JSON - load as block editor format
