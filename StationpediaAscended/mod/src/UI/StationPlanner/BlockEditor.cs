...
namespace StationpediaAscended.UI.StationPlanner
{
    ...
    public class BlockEditor : MonoBehaviour
    {
        ...
        private const string BULLET_PREFIX = "  <color=#FFFFFF>•</color> "; // Indented white bullet
        ...
        public int CurrentLineIndex
        {
            get
            {
                ...
                if (caretPos > parsedText.Length)
                    caretPos = parsedText.Length;
                if (caretPos < 0)
                    caretPos = 0;
                
                // DEBUG
                StationpediaAscendedMod.Log?.LogInfo($"[BlockEditor] CurrentLineIndex: caretPos={caretPos}, parsedText.Length={parsedText.Length}, parsedText='{parsedText.Replace("\n", "\\n")}'");
                ...
                StationpediaAscendedMod.Log?.LogInfo($"[BlockEditor] CurrentLineIndex returning: {lineIndex}");
                return lineIndex;
            }
        }
        ...
        private void OnTextChanged(string newText)
        {
            ...
            try
            {
                ...
                // CRITICAL: Strip bullet/checkbox prefixes from plain text
                // These are visual prefixes added by GenerateRichText() and should not be stored
                // BULLET_PREFIX renders as "  • " (2 spaces + bullet + space)
                for (int i = 0; i < newLines.Length; i++)
                {
                    string line = newLines[i];
                    
                    // Strip leading spaces first, then bullet, then more spaces
                    // This handles "  • text" -> "text"
                    while (line.Length > 0 && (line[0] == ' ' || line[0] == '•'))
                    {
                        if (line[0] == '•')
                        {
                            // Found bullet, strip it and any following space
                            line = line.Substring(1).TrimStart(' ');
                            break; // Only strip one bullet prefix per line
                        }
                        else
                        {
                            // Strip leading space and continue looking for bullet
                ...
                // Detect which lines were edited and downgrade them (only headers)
                for (int i = 0; i < newLines.Length; i++)
                {
                    bool wasEmpty = (i >= oldLines.Length || string.IsNullOrEmpty(oldLines[i]));
                    bool isNowEmpty = string.IsNullOrEmpty(newLines[i]);
                    
                    if ((i < oldLines.Length && newLines[i] == oldLines[i]) || wasEmpty || isNowEmpty || i >= _lineStyles.Count)
                    {
                        continue;
                    }
                    
                    LineStyle style = _lineStyles[i];
                    if (style == LineStyle.H1 || style == LineStyle.H2 || style == LineStyle.H3)
                    {
                        StationpediaAscendedMod.Log?.LogInfo($"[BlockEditor] Line {i} edited, downgrading from {style} to Normal");
        ...
        public void ApplyStyleToLine(int lineIndex, LineStyle style)
        {
            ...
            // Position caret after bullet prefix when applying bullet style
        ...
        /// Uses the rendered text to correctly calculate position including rich text tags.
        /// </summary>
        private void PositionCaretAfterPrefix(int lineIndex)
        {
            if (_inputField == null || _inputField.textComponent == null) return;
            
            // Force TMP to update mesh so GetParsedText() returns current state
            _inputField.textComponent.ForceMeshUpdate(false, false);
            string parsedText = _inputField.textComponent.GetParsedText();
            
            if (string.IsNullOrEmpty(parsedText)) return;
            
            // Split by newlines to get rendered lines
            ...
            // If this line starts with bullet prefix, position after it
            // BULLET_PREFIX renders as "  • " (2 spaces + bullet + space = 4 chars in parsed text)
            if (lineIndex < lines.Length && lines[lineIndex].StartsWith("  • "))
            {
                charPos += 4; // Skip "  • " prefix (2 spaces + bullet + space)
            }
            ...
            _inputField.ActivateInputField();
            _inputField.selectionFocusPosition = charPos; // Set again after activate to ensure it sticks
        }
