    ...
    public static class SurvivalManualLoader
    {
        ...
        public static void RegisterSurvivalManualPage()
        {
            ...
            try
            {
                ...
                _isRegistered = true;
                ConsoleHelper.Print("[Stationpedia Ascended] Survival Manual page registered");
            }
            catch (Exception ex)
            {
        ...
        private static string LoadMarkdownFile()
        {
            ...
            possiblePaths.Add(Path.Combine(BepInEx.Paths.BepInExRootPath, "scripts", "Guides", "In Game Already", "Stationeers Survival Manual.md"));
        ...
        /// <summary>
        /// Convert a ParsedPart to an OperationalDetail with flattened content
        /// (The renderer doesn't support nested children, so we flatten everything into description)
        /// </summary>
        private static OperationalDetail ConvertPartToOperationalDetail(ParsedPart part)
        {
            var sb = new StringBuilder();
            
            // Add TOC at the top of this part
            if (part.Sections.Count > 0)
            {
                sb.AppendLine("<color=#88CCFF><b>Contents:</b></color>");
            ...
            // Flatten all sections and subsections into the description
            foreach (var section in part.Sections)
            {
                // Section header
                sb.AppendLine($"<color=#FFA500><b>{section.Title}</b></color>");
                sb.AppendLine();
                ...
                foreach (var subSection in section.SubSections)
                {
                    sb.AppendLine($"<color=#CCCCCC><b>{subSection.Title}</b></color>");
                    sb.AppendLine();
                    
                    if (subSection.Content.Count > 0)
                    {
                        foreach (var content in subSection.Content)
                        {
                            sb.AppendLine(content);
                            sb.AppendLine();
                        }
