...
namespace StationpediaAscended.Data
{
    ...
    public static class JsonGuideLoader
    {
        ...
        private static List<GuideDescription> _orderedGuides = new List<GuideDescription>(); // Preserves JSON order
        ...
        {
            _loadedGuides.Clear();
            _orderedGuides.Clear();

            if (data?.guides == null || data.guides.Count == 0)
            {
                ConsoleHelper.Print("[Stationpedia Ascended] No custom guides found in descriptions.json");
                return;
            }
            ...
            foreach (var guide in data.guides)
            {
                ...
                string key = guide.isHeader ? guide.headerKey : guide.guideKey;
                
                if (string.IsNullOrEmpty(key))
                {
                    ConsoleHelper.Print("[Stationpedia Ascended] Skipping guide/header with no key");
                    continue;
                }

                _loadedGuides[key] = guide;
                _orderedGuides.Add(guide); // Preserve JSON array order
                
                if (guide.isHeader)
                {
                    ConsoleHelper.Print($"[Stationpedia Ascended] Loaded section header: {guide.headerTitle}");
                }
                else
                {
                    ConsoleHelper.Print($"[Stationpedia Ascended] Loaded guide: {guide.displayName ?? guide.guideKey}");
                }
            }

            ConsoleHelper.Print($"[Stationpedia Ascended] Loaded {_loadedGuides.Count} custom guides/headers from JSON");
        }
        ...
        public static void RegisterGuidePages()
        {
            ...
            foreach (var kvp in _loadedGuides)
            {
                try
                {
                    ...
                    ConsoleHelper.Print($"[Stationpedia Ascended] Registered guide page: {guide.guideKey}");
                }
                catch (Exception ex)
                {
        ...
        /// <summary>
        /// Get all loaded guides in JSON array order (preserves position for headers)
        /// </summary>
        public static List<GuideDescription> GetAllGuides()
        {
            return _orderedGuides.ToList();
        }
