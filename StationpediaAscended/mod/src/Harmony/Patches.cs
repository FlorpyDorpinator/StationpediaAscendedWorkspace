...
namespace StationpediaAscended.Patches
{
    ...
    public static class HarmonyPatches
    {
        ...
        #region KeyManager Keybinding Patch

        /// <summary>
        /// Postfix patch for KeyManager.SetupKeyBindings - adds custom mod keybindings to game settings.
        /// This allows the Station Notepad hotkey to be configured in Settings → Controls.
        /// </summary>
        public static void KeyManager_SetupKeyBindings_Postfix()
        {
            try
            {
                // ALWAYS ensure ControlsGroup exists (not persisted, only KeyItems are saved)
                ControlsGroup modGroup = ControlsGroup.AllControlGroups.Find(g => g.Name == "Stationpedia Ascended");
                if (modGroup == null)
                {
                    modGroup = new ControlsGroup("Stationpedia Ascended");
                }
                
                // Check if KeyItem already exists (may be loaded from settings.xml)
                KeyItem notepadKey;
                if (!KeyManager.KeyItemLookup.TryGetValue("Toggle Station Notepad", out notepadKey))
                {
                    // Create new KeyItem (default F8 - F5 is QuickSave)
                    notepadKey = new KeyItem("Toggle Station Notepad", KeyCode.F8, false);
                    KeyManager.KeyItemLookup["Toggle Station Notepad"] = notepadKey;
                    if (!KeyManager.AllKeys.Contains(notepadKey))
                    {
                        KeyManager.AllKeys.Add(notepadKey);
                    }
                }
                
                // Ensure KeyItem is associated with our group
                if (!modGroup.KeyItems.Contains(notepadKey))
                {
                    modGroup.KeyItems.Add(notepadKey);
                }
                
                // Update _controlsGroupLookup via reflection
                try
                {
                    var controlsGroupLookupField = typeof(KeyManager).GetField("_controlsGroupLookup", 
                        System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static);
                    if (controlsGroupLookupField != null)
                    {
                        var lookup = controlsGroupLookupField.GetValue(null) as Dictionary<string, ControlsGroup>;
                        if (lookup != null)
                        {
                            lookup["Toggle Station Notepad"] = modGroup;
                        }
                    }
                }
                catch { }
        ...
        /// <summary>
        /// Prefix patch for KeyManager.GetButtonDown - blocks the Chatting key (Enter/Return)
        /// when the Station Notepad is open to prevent the chat window from opening.
        /// </summary>
        public static bool KeyManager_GetButtonDown_Prefix(KeyCode key, ref bool __result)
        {
            // Only intercept the Return/Enter keys when Station Notepad is open
            if (key == KeyCode.Return || key == KeyCode.KeypadEnter)
            {
                if (StationpediaAscended.UI.StationPlanner.StationPlannerWindow.IsOpen)
                {
                    // Block the key press by returning false as the result
                    __result = false;
                    return false; // Skip original method
                }
            }
            return true; // Run original method for all other cases
        }
        ...
        /// <summary>
        /// Load the table prefabs from the AssetBundle.
        /// </summary>
        private static bool LoadTablePrefabs()
        {
            if (_tableCellPrefab != null) return true; // Already loaded
            ...
            try
            {
                ...
                {
                    Path.Combine(Path.GetDirectoryName(typeof(HarmonyPatches).Assembly.Location), TABLE_BUNDLE_NAME),
                    Path.Combine(BepInEx.Paths.BepInExRootPath, "scripts", TABLE_BUNDLE_NAME),
                    Path.Combine(BepInEx.Paths.PluginPath, "StationpediaAscended", TABLE_BUNDLE_NAME),
                };
                ...
                if (_tableBundle == null)
                {
                    ...
                    if (_tableBundle == null)
                    {
                        _tableBundle = AssetBundle.LoadFromFile(bundlePath);
                    }
                }
        ...
        /// <summary>
        /// Create a table using prefabs from AssetBundle for proper UI layout.
        /// Falls back to programmatic creation if prefabs unavailable.
        /// </summary>
        private static void CreateTableElement(RectTransform parent, TMPro.TextMeshProUGUI sourceText, List<TableRow> tableData)
        {
            try
            {
                ...
                // Calculate natural width for each column based on content (no wrapping)
                float[] columnWidths = new float[columnCount];
                float maxTotalWidth = 850f;
                float cellPadding = 20f;  // Padding inside each cell (increased)
                float cellSpacing = 24f;  // Spacing between columns (increased)
                float pixelsPerChar = 8.5f;  // Approximate pixels per character (increased for safety)
                
                // Dynamic max column width based on number of columns
                // Give each column fair share of space, with some extra room for uneven content
                float availableForColumns = maxTotalWidth - (columnCount - 1) * cellSpacing;
                float maxSingleColumnWidth = (availableForColumns / columnCount) * 1.5f;  // Allow 50% extra for longer columns
                
                foreach (var row in tableData)
                {
                    if (row?.cells == null) continue;
                    for (int i = 0; i < columnCount && i < row.cells.Count; i++)
                    {
                        float width = ((row.cells[i]?.Length ?? 0) * pixelsPerChar) + cellPadding;
                        if (width > columnWidths[i]) columnWidths[i] = width;
                    }
                }
                
                // First pass: cap any oversized columns
                for (int i = 0; i < columnWidths.Length; i++)
                {
                    if (columnWidths[i] > maxSingleColumnWidth) 
                        columnWidths[i] = maxSingleColumnWidth;
                }
                
                // Calculate total width after capping
                float totalNaturalWidth = 0f;
                for (int i = 0; i < columnWidths.Length; i++)
                {
                    totalNaturalWidth += columnWidths[i];
                }
                totalNaturalWidth += (columnCount - 1) * cellSpacing;  // Add spacing between columns
                
                float minColWidth = 50f;
                
                // Only shrink columns if total width exceeds max
                if (totalNaturalWidth > maxTotalWidth)
                {
                    // Scale all columns proportionally to fit
                    float scaleFactor = availableForColumns / (totalNaturalWidth - (columnCount - 1) * cellSpacing);
                    for (int i = 0; i < columnWidths.Length; i++)
                    {
                        columnWidths[i] = Math.Max(columnWidths[i] * scaleFactor, minColWidth);
                    }
                }
                else
                {
                    // Ensure minimum width but don't cap maximum
                    for (int i = 0; i < columnWidths.Length; i++)
                    {
                        if (columnWidths[i] < minColWidth) columnWidths[i] = minColWidth;
                    }
                }
                
                Color headerColor = VanillaModeManager.IsVanillaMode ? Color.white : new Color(1f, 0.65f, 0f);
                Color separatorColor = new Color(0.4f, 0.4f, 0.4f, 1f);
                
                // Determine if wrapping is needed (only when columns had to be shrunk)
                bool needsWrapping = totalNaturalWidth > maxTotalWidth;
                
                // Always use programmatic table creation (prefabs abandoned)
                CreateTableProgrammatically(parent, sourceText, tableData, columnCount, columnWidths,
                                           headerColor, Color.clear, separatorColor, Color.clear, cellSpacing, maxTotalWidth, needsWrapping);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Create table using loaded prefabs from AssetBundle.
        /// Following strict prefab instantiation rules - DO NOT MODIFY.
        /// </summary>
        private static void CreateTableFromPrefabs(RectTransform parent, TMPro.TextMeshProUGUI sourceText, 
            List<TableRow> tableData, int columnCount, float[] columnWidths,
            Color headerColor, Color separatorColor)
        {
            // DEBUG: Log parent scale to check for scaling issues
            StationpediaAscendedMod.Log.LogInfo($"[TABLE DEBUG] Parent scale: {parent.localScale}, lossyScale: {parent.lossyScale}");
            StationpediaAscendedMod.Log.LogInfo($"[TABLE DEBUG] Parent rect: {parent.rect.width} x {parent.rect.height}");
            
            // 1. Instantiate TableContainer - use SetParent with false (never preserve world position)
            var tableContainer = UnityEngine.Object.Instantiate(_tableContainerPrefab);
            tableContainer.name = "TableContainer";
            tableContainer.transform.SetParent(parent, false);
            
            // FORCE scale to 1,1,1 in case prefab or parent is causing issues
            tableContainer.transform.localScale = Vector3.one;
            
            // 2. Enforce RectTransform anchors on TableContainer
            RectTransform rt = tableContainer.GetComponent<RectTransform>();
            rt.anchorMin = new Vector2(0, 1);
            rt.anchorMax = new Vector2(1, 1);
            rt.pivot = new Vector2(0.5f, 1);
            rt.anchoredPosition = Vector2.zero;
            rt.sizeDelta = new Vector2(0, 0);  // Let layout system determine size
            
            // DEBUG: Log container scale after setup
            StationpediaAscendedMod.Log.LogInfo($"[TABLE DEBUG] Container scale after setup: {tableContainer.transform.localScale}, lossyScale: {tableContainer.transform.lossyScale}");
            
            // 3. REMOVE any existing LayoutElement that might constrain size
            var existingLE = tableContainer.GetComponent<UnityEngine.UI.LayoutElement>();
            if (existingLE != null)
            {
                StationpediaAscendedMod.Log.LogInfo($"[TABLE DEBUG] Removing existing LayoutElement from container");
                UnityEngine.Object.Destroy(existingLE);
            }
            
            // 4. Add ContentSizeFitter to make container expand to fit children
            var tableFitter = tableContainer.GetComponent<UnityEngine.UI.ContentSizeFitter>();
            if (tableFitter == null) tableFitter = tableContainer.AddComponent<UnityEngine.UI.ContentSizeFitter>();
            tableFitter.horizontalFit = UnityEngine.UI.ContentSizeFitter.FitMode.PreferredSize;
            tableFitter.verticalFit = UnityEngine.UI.ContentSizeFitter.FitMode.PreferredSize;
            
            // 5. Check for VerticalLayoutGroup and configure it
            var vlg = tableContainer.GetComponent<UnityEngine.UI.VerticalLayoutGroup>();
            if (vlg != null)
            {
                StationpediaAscendedMod.Log.LogInfo($"[TABLE DEBUG] Container has VLG - childForceExpandHeight: {vlg.childForceExpandHeight}, childControlHeight: {vlg.childControlHeight}");
            ...
            // 6. Process table data - first row is header
            for (int rowIndex = 0; rowIndex < tableData.Count; rowIndex++)
            {
                ...
                // 5. Instantiate row prefab - TableHeaderRow for first row, TableRow for others
                GameObject rowPrefab = isHeader ? _tableHeaderRowPrefab : _tableRowPrefab;
                if (rowPrefab == null) rowPrefab = _tableRowPrefab; // fallback
                
                var row = UnityEngine.Object.Instantiate(rowPrefab);
                row.name = isHeader ? "TableHeaderRow" : $"TableRow_{rowIndex}";
                row.transform.SetParent(tableContainer.transform, false);
                row.transform.localScale = Vector3.one;
                
                // Fix row layout - must expand height to fit tallest cell
                var rowHLG = row.GetComponent<UnityEngine.UI.HorizontalLayoutGroup>();
                if (rowHLG != null)
                {
                    rowHLG.childForceExpandWidth = false;
                    rowHLG.childForceExpandHeight = true;  // Force cells to same height
                    rowHLG.childControlWidth = false;      // We control width via LayoutElement
                    rowHLG.childControlHeight = true;
                }
                
                // Add ContentSizeFitter to row so it expands to fit content
                var rowFitter = row.GetComponent<UnityEngine.UI.ContentSizeFitter>();
                if (rowFitter == null) rowFitter = row.AddComponent<UnityEngine.UI.ContentSizeFitter>();
                rowFitter.horizontalFit = UnityEngine.UI.ContentSizeFitter.FitMode.PreferredSize;
                rowFitter.verticalFit = UnityEngine.UI.ContentSizeFitter.FitMode.PreferredSize;
                
                // 6. Create cells for this row
                for (int colIndex = 0; colIndex < columnCount; colIndex++)
                {
                    string cellText = (colIndex < rowData.cells.Count ? rowData.cells[colIndex] : "") ?? "";
                    
                    // Instantiate TableCell
                    var cell = UnityEngine.Object.Instantiate(_tableCellPrefab);
                    cell.name = $"TableCell_{colIndex}";
                    cell.transform.SetParent(row.transform, false);
                    cell.transform.localScale = Vector3.one;
                    
                    // Add VerticalLayoutGroup to cell for proper text sizing (if not present)
                    var cellLayout = cell.GetComponent<UnityEngine.UI.VerticalLayoutGroup>();
                    if (cellLayout == null)
                    {
                        cellLayout = cell.AddComponent<UnityEngine.UI.VerticalLayoutGroup>();
                        cellLayout.padding = new RectOffset(6, 6, 4, 4);
                    ...
                    // 7. Set column width on cell LayoutElement
                    var cellLE = cell.GetComponent<UnityEngine.UI.LayoutElement>();
                    if (cellLE == null) cellLE = cell.AddComponent<UnityEngine.UI.LayoutElement>();
                    cellLE.preferredWidth = columnWidths[colIndex];
                    cellLE.minWidth = columnWidths[colIndex];  // Also set min to prevent shrinking
                    cellLE.flexibleWidth = 0;
                    cellLE.minHeight = 24f; // Minimum cell height
                    
                    // Add ContentSizeFitter to cell for vertical sizing
                    var cellFitter = cell.GetComponent<UnityEngine.UI.ContentSizeFitter>();
                    if (cellFitter == null) cellFitter = cell.AddComponent<UnityEngine.UI.ContentSizeFitter>();
                    cellFitter.horizontalFit = UnityEngine.UI.ContentSizeFitter.FitMode.Unconstrained;  // Width controlled by LayoutElement
                    cellFitter.verticalFit = UnityEngine.UI.ContentSizeFitter.FitMode.PreferredSize;    // Height expands to fit text
                    
                    // 8. Find TMP text inside cell
                    var textComponent = cell.GetComponentInChildren<TMPro.TextMeshProUGUI>();
                    if (textComponent != null)
                    {
                        textComponent.font = sourceText.font;
                        textComponent.fontSharedMaterial = sourceText.fontSharedMaterial;
                        textComponent.fontSize = 16f; // Slightly smaller for better fit
                        textComponent.enableAutoSizing = false;
                        textComponent.overflowMode = TMPro.TextOverflowModes.Truncate; // Truncate if too long
                        textComponent.enableWordWrapping = true;
                        textComponent.transform.localScale = Vector3.one;
                        
                        if (isHeader)
                        {
                            textComponent.text = $"<b>{cellText}</b>";
                            textComponent.color = headerColor;
                        }
                        else
                        {
                            textComponent.text = cellText;
                            textComponent.color = sourceText.color;
                        }
                    }
                }
                
                // 9. Add TableHeaderSeparator immediately after TableHeaderRow
                if (isHeader && _tableSeparatorPrefab != null)
                {
                    var separator = UnityEngine.Object.Instantiate(_tableSeparatorPrefab);
                    separator.name = "TableHeaderSeparator";
                    separator.transform.SetParent(tableContainer.transform, false);
                    // NEVER set localScale
                    
                    // Set separator color
                    var sepImage = separator.GetComponent<UnityEngine.UI.Image>();
                    if (sepImage != null)
                    {
                        sepImage.color = separatorColor;
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Create table programmatically - the working version without prefabs.
        /// </summary>
        private static void CreateTableProgrammatically(RectTransform parent, TMPro.TextMeshProUGUI sourceText,
            List<TableRow> tableData, int columnCount, float[] columnWidths,
            Color headerColor, Color headerBgColor, Color separatorColor, Color cellBgColor,
            float cellSpacing, float maxTotalWidth, bool needsWrapping)
        {
            var tableGO = new GameObject("TableContainer");
            tableGO.transform.SetParent(parent, false);
            
            var tableRect = tableGO.AddComponent<RectTransform>();
            ...
            var tableBg = tableGO.AddComponent<UnityEngine.UI.Image>();
            tableBg.color = Color.clear;
            
            var tableLayout = tableGO.AddComponent<UnityEngine.UI.VerticalLayoutGroup>();
            ...
            var tableFitter = tableGO.AddComponent<UnityEngine.UI.ContentSizeFitter>();
            tableFitter.horizontalFit = UnityEngine.UI.ContentSizeFitter.FitMode.PreferredSize;
            tableFitter.verticalFit = UnityEngine.UI.ContentSizeFitter.FitMode.PreferredSize;
            
            var tableLayoutElement = tableGO.AddComponent<UnityEngine.UI.LayoutElement>();
            tableLayoutElement.preferredWidth = maxTotalWidth;
            
            // Calculate total row width for separator
            float totalRowWidth = 0;
            for (int i = 0; i < columnWidths.Length; i++)
            {
                totalRowWidth += columnWidths[i];
            }
            totalRowWidth += (columnCount - 1) * cellSpacing + 12; // spacing + row padding
            ...
            for (int rowIndex = 0; rowIndex < tableData.Count; rowIndex++)
            {
                ...
                var rowGO = new GameObject(isHeader ? "HeaderRow" : $"Row{rowIndex}");
                ...
                rowLayout.padding = new RectOffset(6, 6, isHeader ? 2 : 4, isHeader ? 0 : 4);
                ...
                for (int colIndex = 0; colIndex < columnCount; colIndex++)
                {
                    ...
                    var cellGO = new GameObject($"Cell{colIndex}");
                    ...
                    var cellBg = cellGO.AddComponent<UnityEngine.UI.Image>();
                    cellBg.color = Color.clear;
                    
                    // Use VerticalLayoutGroup so cell expands to fit text
                    var cellLayout = cellGO.AddComponent<UnityEngine.UI.VerticalLayoutGroup>();
                    cellLayout.padding = new RectOffset(6, 6, 3, 3);
                    ...
                    var cellLayoutElement = cellGO.AddComponent<UnityEngine.UI.LayoutElement>();
                    cellLayoutElement.minWidth = 50f;
                    cellLayoutElement.preferredWidth = columnWidths[colIndex];
                    cellLayoutElement.flexibleWidth = 0f;
                    cellLayoutElement.minHeight = 24f;
                    
                    // ContentSizeFitter makes cell expand vertically based on text
                    ...
                    if (isHeader)
                    {
                        // For headers: allow wrapping only if there are multiple words AND we need wrapping
                        bool hasMultipleWords = cellText.Contains(" ");
                        textComponent.enableWordWrapping = needsWrapping && hasMultipleWords;
                        textComponent.text = $"<b>{cellText}</b>";
                        textComponent.color = headerColor;
                    }
                    else
                    {
                        // For data cells: only wrap if we need to (table too wide)
                        if (needsWrapping)
                        {
                            bool hasMultipleWords = cellText.Contains(" ");
                            if (hasMultipleWords)
                            {
                                // Multi-word: allow wrapping at word boundaries
                                textComponent.enableWordWrapping = true;
                            }
                            else
                            {
                                // Single word: use auto-sizing to shrink rather than break mid-word
                                textComponent.enableWordWrapping = false;
                                textComponent.enableAutoSizing = true;
                                textComponent.fontSizeMin = sourceText.fontSize * 0.6f;
                                textComponent.fontSizeMax = sourceText.fontSize;
                            }
                        }
                        else
                        {
                            // Table fits - no wrapping needed
                            textComponent.enableWordWrapping = false;
                        }
                        textComponent.text = cellText;
                        textComponent.color = sourceText.color;
                ...
                // Add separator after header with constrained width
                if (isHeader)
                {
                    var sepGO = new GameObject("Separator");
                    sepGO.transform.SetParent(tableGO.transform, false);
                    
                    var sepRect = sepGO.AddComponent<RectTransform>();
                    
                    var sepImage = sepGO.AddComponent<UnityEngine.UI.Image>();
                    sepImage.color = separatorColor;
                    
                    var sepLayout = sepGO.AddComponent<UnityEngine.UI.LayoutElement>();
                    sepLayout.minHeight = 2f;
                    sepLayout.preferredHeight = 2f;
                    sepLayout.flexibleHeight = 0f;
                    sepLayout.preferredWidth = totalRowWidth;
                    sepLayout.flexibleWidth = 0f;
                    
                    // Add spacer after separator to create gap before first data row
                    var spacerGO = new GameObject("HeaderSpacer");
                    spacerGO.transform.SetParent(tableGO.transform, false);
                    var spacerRect = spacerGO.AddComponent<RectTransform>();
                    var spacerLayout = spacerGO.AddComponent<UnityEngine.UI.LayoutElement>();
                    spacerLayout.minHeight = 6f;
                    spacerLayout.preferredHeight = 6f;
        ...
        private static void PopulateGameMechanicsContents(Stationpedia stationpedia)
        {
            ...
            foreach (var mechanic in mechanics)
            {
                ...
                {
                    string headerName = $"SectionHeader_{mechanic.headerKey ?? mechanic.guideKey}";
                    bool headerExists = false;
                    foreach (Transform child in stationpedia.LoreGuideContents)
                    {
                        if (child.gameObject.name == headerName)
                        {
                            headerExists = true;
                            break;
                        }
                    ...
                    if (!headerExists)
                    {
                        CreateSectionHeader(stationpedia.LoreGuideContents, mechanic.headerTitle ?? "Section", headerName);
                    }
                    continue;
                }
        ...
        private static void CreateJsonGuideButtons(Stationpedia stationpedia)
        {
            try
            {
                ...
                foreach (var guide in guides)
                {
                    ...
                    if (guide.isHeader)
                    {
                        string headerName = $"SectionHeader_{guide.headerKey ?? guide.guideKey}";
                        bool headerExists = false;
                        foreach (Transform child in stationpedia.LoreGuideContents)
                        {
                            if (child.gameObject.name == headerName)
                        ...
                        if (!headerExists)
                        {
                            CreateSectionHeader(stationpedia.LoreGuideContents, guide.headerTitle ?? "Section", headerName);
                            createdCount++;
                        }
                        continue;
                    }
        ...
        /// <summary>
        /// Create a section header (non-clickable text divider) in the guides list
        /// </summary>
        private static void CreateSectionHeader(Transform parent, string headerText, string headerName)
        {
            try
            {
                // Create a simple text header
                var headerGO = new GameObject(headerName);
                ...
                var layoutElement = headerGO.AddComponent<UnityEngine.UI.LayoutElement>();
                layoutElement.minHeight = 30;
                layoutElement.preferredHeight = 30;
                layoutElement.flexibleWidth = 1;
                
                // Add TextMeshPro text - copy font from LoreGuideTitle to match "Game Mechanics" style
                var textComponent = headerGO.AddComponent<TMPro.TextMeshProUGUI>();
                
                // Get the LoreGuideTitle font to match the style
                if (Stationpedia.Instance?.LoreGuideTitle != null)
                {
                    textComponent.font = Stationpedia.Instance.LoreGuideTitle.font;
                    textComponent.fontSharedMaterial = Stationpedia.Instance.LoreGuideTitle.fontSharedMaterial;
                }
                
                textComponent.text = headerText;
                textComponent.fontSize = 18; // Slightly smaller than "Game Mechanics" title
                textComponent.fontStyle = TMPro.FontStyles.Normal;
                textComponent.alignment = TMPro.TextAlignmentOptions.Left;
                textComponent.color = Stationpedia.Instance?.LoreGuideTitle?.color ?? new UnityEngine.Color(0.85f, 0.85f, 0.85f, 1f);
                textComponent.margin = new Vector4(15, 10, 10, 5);
