...
namespace StationpediaAscended.Patches
{
    ...
    public static class SearchPatches
    {
        ...
        private static void BuildPageIndexes()
        {
            ...
            stopwatch.Stop();
            ConsoleHelper.Print($"[Stationpedia Ascended] Built page indexes: {_pageTitleIndex.Count} titles, {_pageWordIndex.Count} words in {stopwatch.ElapsedMilliseconds}ms");
        }
        ...
        private static void BuildCategoryCache()
        {
            ...
            {
                Debug.LogWarning($"[Stationpedia Ascended] Error building category cache: {ex.Message}");
            }
            
            _categoryCacheBuilt = true;
            stopwatch.Stop();
            ConsoleHelper.Print($"[Stationpedia Ascended] Built category cache: {_categoryCache.Count} entries in {stopwatch.ElapsedMilliseconds}ms");
        }
