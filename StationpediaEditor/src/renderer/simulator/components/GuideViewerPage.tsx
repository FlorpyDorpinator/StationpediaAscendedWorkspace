/**
 * GuideViewerPage component
 * Displays a single guide's markdown content
 */
import React, { useState, useEffect } from 'react';

export interface GuideViewerPageProps {
  guidePath: string;
  guideName: string;
  onBack?: () => void;
...
const CollapsibleSection: React.FC<{
  ...
  level?: number;
}> = ({ title, children, defaultExpanded = false, level = 1 }) => {
...
function markdownToJsx(content: string): React.ReactNode {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Convert inline code
    line = line.replace(/`([^`]+)`/g, '<code class="bg-[#2A2F34] px-1 rounded text-[#FF7A18]">$1</code>');
...
export const GuideViewerPage: React.FC<GuideViewerPageProps> = ({
  guidePath,
  guideName,
  onBack,
  onNavigate,
  ...
  useEffect(() => {
    async function loadGuide() {
      try {
        ...
        const result = await window.electronAPI.readFile(guidePath);
        if (!result.success || !result.data) {
          throw new Error(result.error || 'Failed to read guide');
        }
